...
int main(){
   //srand() has to run once per program run
    srand(time(0)); // Seed , starting point
    ...
    while (!end) { //while true
        ...
        size_t rand_num = static_cast<size_t>((rand() % 11)); //sabit listeleri gibi sayısal veri türlerini int'lere veya int'leri float'lara dönüştürmek istediğinizde kullanırsınız static_cast
